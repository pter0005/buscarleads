...
import { Download, Zap, Star, RefreshCw, Sparkles, Globe, Loader2 } from 'lucide-react'
  ...
  const handleSearch = useCallback(async (nicho: string, cidade: string) => {
    ...
    try {
      ...
      if (data.leads.length === 0) {
        setErrorMsg('Nenhum resultado encontrado. Tente outro nicho ou cidade.')
      } else {
        setLeads(data.leads)
      ...
      setErrorMsg(e.message || 'Erro inesperado. Verifique sua APIFY_TOKEN.')
    } finally {
      setLoading(false)
    }
  }, [])
    ...
    <div className="min-h-screen bg-surface">
      ...
      <nav className="sticky top-0 z-30 bg-surface/80 backdrop-blur-md border-b border-surface-border">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href="https://newperfect.netlify.app" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-surface-border bg-surface-card text-gray-400 hover:border-brand/30 hover:text-brand transition-all duration-200">
              <Globe className="w-3.5 h-3.5" /> NEW Agency
            </a>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm max-w-2xl">
            Busque qualquer nicho em qualquer cidade via Apify. O chat no canto inferior direito entende linguagem natural — só falar o que você quer.
          </p>
        </div>
        ...
        {/* Loading overlay */}
        {loading && (
          <div className="flex items-center gap-3 bg-brand/10 border border-brand/20 rounded-xl px-4 py-3 mb-4 text-sm text-brand">
            <Loader2 className="w-4 h-4 animate-spin" />
            Buscando no Apify... isso pode levar até 30 segundos.
          </div>
        )}
        ...
        {searchMeta && !loading && (
          <div className="flex items-center gap-3 mb-4">
            <div className="flex items-center gap-2 text-xs text-brand bg-brand/10 border border-brand/20 px-3 py-1.5 rounded-full">
              ...
              <strong>{searchMeta.nicho}</strong> em <strong>{searchMeta.cidade}</strong> via Apify
            </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setFilters({ busca: '', semSite: false, semInstagram: false, scoreMin: 0, status: '', cidade: '' })}
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <h2 className="text-xl font-bold text-white mb-1">
              Transformamos negócios locais em <span className="text-brand">marcas digitais</span>
            </h2>
          ...
          <div className="flex flex-col gap-2 shrink-0">
            <a href="https://newperfect.netlify.app" target="_blank" rel="noopener noreferrer"
              ...
              <Globe className="w-4 h-4" /> Ver portfólio
            </a>
      ...
      {/* AI Chat flutuante */}
      <AiChat onSearch={handleSearch} loading={loading} />
    </div>
  )
}
